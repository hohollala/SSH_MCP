...
async function testSSHConnectWithSSHenv() {
  console.log('MCP SSH 연결 테스트 (.sshenv 파일 사용)...');
  ...
  try {
    ...
    // .sshenv 파일의 test 서버 설정 사용
    const connectResult = await mcpServer.executeTool('ssh_connect', {
      host: '158.180.71.210',
      username: 'ubuntu',
      privateKeyPath: 'E:\\AIProjects\\ssh_Test\\docs\\first.pem', // 실제 파일 경로 사용
      port: 22
    });
...
// 테스트 실행
testSSHConnectWithSSHenv().catch(console.error); 